...
from .model_acm_net import LSTMContext_acm, FrequencyBias_acm  ## acmnet
...
@registry.ROI_RELATION_PREDICTOR.register("ACMPredictor")
class ACMPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(ACMPredictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        if self.attribute_on:
            ...
            self.context_layer = LSTMContext_acm(config, obj_classes, rel_classes, in_channels)
        ...
        if self.use_bias:
            # convey statistics into FrequencyBias to avoid loading again
            self.freq_bias = FrequencyBias_acm(config, statistics)

        # #xk# 用于转换一下4096维特征，不然是直接concat了
        self.norm_liner = nn.Sequential(
            nn.Linear(self.pooling_dim, self.pooling_dim),
            nn.BatchNorm1d(self.pooling_dim),
            nn.ReLU()
        )
        self.reweight_liner = nn.Sequential(
            nn.Linear(self.pooling_dim, self.pooling_dim),
            nn.BatchNorm1d(self.pooling_dim),
            nn.ReLU()
        )
        self.mix_classifier = nn.Linear(self.pooling_dim * 2, self.num_rel_cls, bias=True)
        layer_init(self.norm_liner[0], normal=True)
        layer_init(self.reweight_liner[0], normal=True)
        layer_init(self.mix_classifier, xavier=True)

    def current_iter(self, iter):  # xk# jia
        self.iter = iter
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.training:
            ...
            reweight_input = prod_rep.clone()
            # reweight_label = rel_labels.clone()
            reweight_pair = pair_pred.clone()

            l = ((self.current_iter // 1250) / (60000 // 1250) - 1) ** 2  # our decay   ## (x / 48 - 1)**2

            norm_input_mapped = self.norm_liner(prod_rep)
            reweight_input_mapped = self.reweight_liner(reweight_input)
            mixed_input = 2 * torch.cat((l * norm_input_mapped, (1 - l) * reweight_input_mapped), dim=1)

            rel_dists = self.mix_classifier(mixed_input)
        else:
            norm_input_mapped = self.norm_liner(prod_rep)
            reweight_input_mapped = self.reweight_liner(prod_rep)
            prod_rep = torch.cat((norm_input_mapped, reweight_input_mapped), dim=1)  # xk# add

            rel_dists = self.mix_classifier(prod_rep)
        ...
        if self.attribute_on:
            ...
            return obj_dists, rel_dists, add_losses, l
...
class MotifPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        # rel_dists = self.rel_compress(prod_rep) #xk# yuan relation

        '''begin to change'''  #xk#
        l = None
        if self.training:
            # norm_input = prod_reps  ## 原始输入，这里不改名字
            # norm_label = rel_labels
            # norm_pairs = pair_pred
            ...
            norm_input_mapped = self.norm_liner(prod_rep)
            reweight_input_mapped = self.reweight_liner(prod_rep)
            prod_rep = torch.cat((norm_input_mapped, reweight_input_mapped), dim=1)  # xk#加
        ...
        if self.use_bias:
            rel_dists = rel_dists + self.freq_bias.index_with_labels(pair_pred.long())  #xk# yuan
            # rel_dists = rel_dists + l * self.freq_bias.index_with_labels(pair_pred.long()) + \
            #             (1 - l) * self.freq_bias.index_with_labels(reweight_pair.long())  # xk# gai
        ...
        if self.attribute_on:
            ...
            # return obj_dists, rel_dists, add_losses, l #xk# yuan
            return obj_dists, rel_dists, add_losses, l #xk# gai
...
class VCTreePredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.training:
            ...
            # l = ((self.current_iter // 1250) / (60000 // 1250) - 1) ** 2  # our decay   ## (x / 48 - 1)**2
            # l = ((self.current_iter // 2500) / (60000 // 2500) - 1) ** 2  # our decay   ## (x / 24 - 1)**2
            l = ((self.current_iter // 2500) / (30000 // 2500) - 1) ** 2  # our decay   ## (x / 24 - 1)**2
        ...
        # return obj_dists, rel_dists, add_losses
        return obj_dists, rel_dists, add_losses, l #xk#
