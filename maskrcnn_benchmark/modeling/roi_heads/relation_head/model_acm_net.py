...
from .utils_motifs import obj_edge_vectors, center_x, sort_by_score, to_onehot, get_dropout_mask, nms_overlaps, \
    encode_box_info, transpose_packed_sequence_inds


class FrequencyBias_acm(nn.Module):
    """
    ...
    def __init__(self, cfg, statistics, eps=1e-3):
        super(FrequencyBias_acm, self).__init__()
...
class LSTMContext_acm(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        ##  不对vis_feat 进行压缩 xk jia
        self.obj_sel_pos = nn.Sequential(*[
            nn.Linear(4424, 80), nn.ReLU(inplace=True),
        ])
        self.pos_sel_obj = nn.Sequential(
            nn.Linear(4424, 80), nn.ReLU(inplace=True),
        )

        self.rel_sel_pos = nn.Sequential(*[
            nn.Linear(4808, 80), nn.ReLU(inplace=True),
        ])
        self.pos_sel_rel = nn.Sequential(
            nn.Linear(4808, 80), nn.ReLU(inplace=True),
        )
    ...
    def obj_ctx(self, obj_feats, proposals, obj_labels=None, boxes_per_cls=None, ctx_average=False):
        ...
        # sss = torch.any(torch.isnan(obj_inp_rep))
    ...
    def obj_ctx_with_random_fixed_seque(self, obj_feats, proposals, vis_feature, pos_embed, obj_embed, obj_labels=None, boxes_per_cls=None, ctx_average=False):
        num_rois = [len(b) for b in proposals]
        num_rois_tensor = torch.Tensor(num_rois).to(vis_feature.device)
        num_im = len(num_rois)

        _, l2s_idx = torch.sort(num_rois_tensor, descending=True)
        l2s_idx = l2s_idx.cpu().tolist()
        _perm = []
        tmp_num = 0  ## 将每张图片的到的序列拼起来的时候用
        for i in range(num_im):
            _index = torch.eye(num_rois[i]).to(obj_feats.device)
            _index = torch.argmax(_index, -1)
            # random_perm = _index[torch.randperm(num_rois[i])]
            random_perm = _index
            _perm.append(random_perm+tmp_num)
            tmp_num = tmp_num + num_rois[i]

        perm = []
        for i in l2s_idx:
            perm.append(_perm[i])
        num_rois = sorted(num_rois, reverse=True)  # xk# 降序
        # for i in range(num_im):
        #     perm[num_im-i-1] = perm[num_im-i-1] + i * num_rois[num_im-i-1]
        perm = cat(perm).view(-1)

        inds, ls_transposed = transpose_packed_sequence_inds(num_rois)
        inds = torch.LongTensor(inds).to(obj_feats.device)
        ...
        ## 以下 为学习seque 进行ctx
        obj_fea_584 = obj_feats  ## 使用原来的feature，自己不组合
        num_rois_ctx = [len(b) for b in proposals]
        num_rois_ctx_tensor = torch.Tensor(num_rois_ctx).to(vis_feature.device)
        num_im_ctx = len(num_rois_ctx)
        obj_fea_584 = obj_fea_584.split(num_rois_ctx, dim=0)
        obj_fea_584_padded = []
        for i in range(num_im_ctx):
            expand_zero = torch.zeros((80 - obj_fea_584[i].shape[0], obj_fea_584[i].shape[1])).to(obj_fea_584[i].device)  ## 预设框 80
            obj_fea_584_padded.append(cat([obj_fea_584[i], expand_zero], 0))

        obj_fea_584 = torch.stack(obj_fea_584_padded, 0)
        obj_sel_pos_matrix = self.obj_sel_pos(obj_fea_584)
        pos_sel_obj_matrix = self.pos_sel_obj(obj_fea_584)

        # #1# 先剪再softmax
        _, l2s_idx_ctx = torch.sort(num_rois_ctx_tensor, descending=True)
        l2s_idx_ctx = l2s_idx_ctx.cpu().tolist()
        _perm_ctx = []
        tmp_num_ctx = 0  ## 将每张图片的到的序列拼起来的时候用
        for i in range(num_im_ctx):
            tmp_obj_atten = F.softmax(obj_sel_pos_matrix[i, :num_rois_ctx[i], :num_rois_ctx[i]], dim=1)
            tmp_pos_atten = F.softmax(pos_sel_obj_matrix[i, :num_rois_ctx[i], :num_rois_ctx[i]], dim=0)
            # tmp_mix = tmp_obj_atten + tmp_pos_atten  ## 加
            tmp_mix = tmp_obj_atten * tmp_pos_atten  ## 乘
            obj_pos_seq = torch.zeros_like(tmp_mix).to(tmp_mix.device)
            # obj_pos_atten_no_pad.append(tmp_mix)
            for j in range(tmp_mix.shape[0]):
                max_atten = torch.argmax(tmp_mix)
                tmp_mix[max_atten.item() // tmp_mix.shape[0], :] = 0
                tmp_mix[:, max_atten.item() % tmp_mix.shape[0]] = 0
                obj_pos_seq[max_atten.item() // tmp_mix.shape[0]][max_atten.item() % tmp_mix.shape[0]] = 1
            tmp_i_perm = torch.argmax(obj_pos_seq, -1)
            _perm_ctx.append(tmp_i_perm + tmp_num_ctx)
            tmp_num_ctx = tmp_num_ctx + num_rois_ctx[i]


        perm_ctx = []
        for i in l2s_idx_ctx:
            perm_ctx.append(_perm_ctx[i])
        num_rois_ctx = sorted(num_rois_ctx, reverse=True)  # xk# 降序
        perm_ctx = cat(perm_ctx).view(-1)

        inds_ctx, ls_transposed_ctx = transpose_packed_sequence_inds(num_rois_ctx)
        inds_ctx = torch.LongTensor(inds_ctx).to(obj_fea_584.device)
        ls_transposed_ctx = torch.LongTensor(ls_transposed_ctx)
        perm_ctx = perm_ctx[inds_ctx]  # (batch_num_box, )
        _, inv_perm_ctx = torch.sort(perm_ctx)

        # Sort by the confidence of the maximum detection.
        # perm, inv_perm, ls_transposed = self.sort_rois(proposals)  #xk# 这里被我注释掉了
        # Pass object features, sorted by score, into the encoder LSTM
        obj_inp_rep = obj_feats[perm_ctx].contiguous()
        input_packed = PackedSequence(obj_inp_rep, ls_transposed_ctx)
        encoder_rep = self.obj_ctx_rnn(input_packed)[0][0]
        encoder_rep = self.lin_obj_h(encoder_rep)  # map to hidden_dim

        obj_inp_rep = obj_inp_rep[inv_perm_ctx].contiguous()
        encoder_rep = encoder_rep[inv_perm_ctx].contiguous()

        obj_inp_rep = obj_inp_rep[perm].contiguous()
        encoder_rep = encoder_rep[perm].contiguous()
        ## 以上经过ctx，并还原至原顺序  ## 注意这里上两句不可以省略，因为当batch>1时，各张图片的物体框需要交叉排列！！！！
    ...
    def edge_ctx_with_seqatten(self, inp_feats, perm, inv_perm, ls_transposed, proposals):
        """
        Object context and object classification.
        :param obj_feats: [num_obj, img_dim + object embedding0 dim]
        :return: edge_ctx: [num_obj, #feats] For later!
        """
        num_rois = [len(b) for b in proposals]
        num_rois_tensor = torch.Tensor(num_rois).to(inp_feats.device)
        num_im = len(num_rois)
        rel_fea = inp_feats.split(num_rois, dim=0)
        rel_fea_padded = []
        for i in range(num_im):
            expand_zero = torch.zeros((80 - rel_fea[i].shape[0], rel_fea[i].shape[1])).to(rel_fea[i].device)  ## 预设框 80
            rel_fea_padded.append(cat([rel_fea[i], expand_zero], 0))

        rel_fea = torch.stack(rel_fea_padded, 0)
        rel_sel_pos_matrix = self.rel_sel_pos(rel_fea)
        pos_sel_rel_matrix = self.pos_sel_rel(rel_fea)
        ...
        tmp_num = 0  ## 将每张图片的到的序列拼起来的时候用
        for i in range(num_im):
            tmp_obj_atten = F.softmax(rel_sel_pos_matrix[i, :num_rois[i], :num_rois[i]], dim=1)
            tmp_pos_atten = F.softmax(pos_sel_rel_matrix[i, :num_rois[i], :num_rois[i]], dim=0)
            # tmp_mix = tmp_obj_atten + tmp_pos_atten  ## 加
            tmp_mix = tmp_obj_atten * tmp_pos_atten  ## 乘
            obj_pos_seq = torch.zeros_like(tmp_mix).to(tmp_mix.device)

            # import pandas as pd  # #打印学到的矩阵
            # save = torch.cat(
            #     [rel_sel_pos_matrix[i, :num_rois[i], :num_rois[i]], pos_sel_rel_matrix[i, :num_rois[i], :num_rois[i]],
            #      tmp_mix], dim=0)
            # csv = save.squeeze(dim=0).cpu().numpy()
            # csv_pd = pd.DataFrame(csv)
            # writer = pd.ExcelWriter('matrix.xlsx')
            # # np.savetxt("mrtix.csv", csv)
            # csv_pd.to_excel(writer, 'sheet1', float_format='%.6f')
            # writer.save()
            # writer.close()
            ...
            for j in range(tmp_mix.shape[0]):
                ...
                tmp_mix[:, max_atten.item() % tmp_mix.shape[0]] = 0
                obj_pos_seq[max_atten.item() // tmp_mix.shape[0]][max_atten.item() % tmp_mix.shape[0]] = 1
                # print(max_atten.item() // tmp_mix.shape[0])  ## 位置idx
                # print(max_atten.item() % tmp_mix.shape[0])  ## 物体idx ##看顺序，只打开这个注释
            tmp_i_perm = torch.argmax(obj_pos_seq, -1)
            _perm_rel.append(tmp_i_perm + tmp_num)
            tmp_num = tmp_num + num_rois[i]
            # print('*' * 20)
        ...
        perm_rel = []
        for i in l2s_idx:
            perm_rel.append(_perm_rel[i])
        num_rois = sorted(num_rois, reverse=True)  # xk# 降序
        # for i in range(num_im):
        #     perm[num_im-i-1] = perm[num_im-i-1] + i * num_rois[num_im-i-1]
        perm_rel = cat(perm_rel).view(-1)

        inds_rel, ls_transposed_rel = transpose_packed_sequence_inds(num_rois)
        inds_rel = torch.LongTensor(inds_rel).to(rel_fea.device)
        ls_transposed_rel = torch.LongTensor(ls_transposed_rel)
        perm_rel = perm_rel[inds_rel]  # (batch_num_box, )
        _, inv_perm_rel = torch.sort(perm_rel)


        # edge_input_packed = PackedSequence(inp_feats[perm], ls_transposed)  ## 原文
        edge_input_packed = PackedSequence(inp_feats[perm_rel], ls_transposed_rel)  #xk#
        edge_reps = self.edge_ctx_rnn(edge_input_packed)[0][0]
        edge_reps = self.lin_edge_h(edge_reps) # map to hidden_dim
        edge_ctx = edge_reps[inv_perm_rel]
        return edge_ctx
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        # obj_dists, obj_preds, obj_ctx, perm, inv_perm, ls_transposed = self.obj_ctx(obj_pre_rep, proposals, obj_labels, boxes_per_cls, ctx_average=ctx_average)  ##  yuan
        obj_dists, obj_preds, obj_ctx, perm, inv_perm, ls_transposed = self.obj_ctx_with_random_fixed_seque(obj_pre_rep, proposals, x, pos_embed, obj_embed, obj_labels, boxes_per_cls, ctx_average=ctx_average)  # xk#
        ...
        # edge_ctx = self.edge_ctx(obj_rel_rep, perm=perm, inv_perm=inv_perm, ls_transposed=ls_transposed)  ## yuan
        edge_ctx = self.edge_ctx_with_seqatten(obj_rel_rep, perm=perm, inv_perm=inv_perm, ls_transposed=ls_transposed, proposals=proposals) ## xk
